  ...
  if expression.HasField('bin_op'):
    ...
    return {
        ...
        e.DIVIDE:
            lambda: lhs_value / rhs_value,
        e.MAX:
            lambda: max(lhs_value, rhs_value),
        e.MIN:
...
class RegressionEstimator(Estimator):
  ...
  def __init__(self, op, delay_expressions: Sequence[delay_model_pb2.DelayExpression],
    ...
    self._k_fold_cross_validation(self.raw_data_points, num_cross_validation_folds,
  ...
  def generate_k_fold_cross_validation_train_and_test_sets(
      raw_data_points: Sequence[Tuple[int]],
      num_cross_validation_folds: int):
      """Yields 'num_cross_validation_folds' number of training and
      testing datasets for use in cross validation.
      """

      # Separate data into num_cross_validation_folds sets
      random.seed(0)
      randomized_data_points = random.sample(raw_data_points, len(raw_data_points))
      folds = []
      for fold_idx in range(num_cross_validation_folds):
          folds.append([dp for idx, dp in enumerate(randomized_data_points)
  ...
  def _k_fold_cross_validation(self, raw_data_points: Sequence[Tuple[int]],
                               num_cross_validation_folds: int,
                               max_data_point_error: float,
                               max_fold_geomean_error: float):
      """Perfroms k-fold cross validation to verify the model. An exception is
      raised if the model does not pass cross validation.  Note that this
      function modifies self.delay_function to perform regression on partial
      data sets.
      ...
      if(num_cross_validation_folds > len(raw_data_points)):
          raise Error('{}: Too few data points to cross validate: '
              '{} data points, {} folds'.format(self.op, len(raw_data_points),
...
def _estimator_from_proto(op: Text, proto: delay_model_pb2.Estimator,
  ...
  if proto.HasField('regression'):
    assert data_points
    keyword_dict = dict()
    optional_args = ['num_cross_validation_folds',
                     'max_data_point_error',
                     'max_fold_geomean_error',
                    ]
    for arg in optional_args:
      if proto.regression.HasField(arg):
        keyword_dict[arg] = getattr(proto.regression, arg)
